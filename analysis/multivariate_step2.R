...
step1_1 <- subset(step1_1, nsignificant ==  2)
step1_1 <- step1_1[order(step1_1$rmse),][1:100,]$model
step1_1 <- do.call(rbind, step1_1)

step1_2 <- subset(step1_2, nsignificant ==  2)
step1_2 <- step1_2[order(step1_2$rmse),][1:100,]$model
step1_2 <- do.call(rbind, step1_2)
...
models1 <- cbind(step1_1[models1$idx, ], models1$var3)
models1 <- as.matrix(models1)

vars2 <- c(vars2, "Size")
models2 <- expand.grid(idx = 1:100, var3 = vars2, stringsAsFactors = FALSE)
models2 <- cbind(step1_2[models2$idx, ], models2$var3)
models2 <- as.matrix(models2)


# Filtering out cases
models1 <- models1[(models1[,1] != models1[,3]) & (models1[,2] != models1[,3]),]
models2 <- models2[(models2[,1] != models2[,3]) & (models2[,2] != models2[,3]),]

# # Filtering out cases
# models_main_vars1 <- models1
# models_main_vars1[] <- gsub("(Min|Max|Avg\\.|Geom\\.|Range).+$", "" , models_main_vars1)
# 
# test              <- which(models_main_vars1[,2] != models_main_vars1[,3])
# models_main_vars1 <- models_main_vars1[test,]
# models1           <- models1[test, ]
# 
# test              <- which(models_main_vars1[,1] != models_main_vars1[,3])
# models_main_vars1 <- models_main_vars1[test,]
# models1           <- models1[test, ]
# 
...
# 
# test              <- which(models_main_vars2[,1] != models_main_vars2[,3])
# models_main_vars2 <- models_main_vars2[test,]
# models2           <- models2[test, ]
# 
# test              <- which(models_main_vars2[,2] != models_main_vars2[,3])
# models_main_vars2 <- models_main_vars2[test,]
# models2           <- models2[test, ]
...
tabulate_counts(
  ...
  caption.     = "Top 20 variables with 3 predictors for CI in time 1."
)
...
tabulate_counts(
  ...
  caption.     = "Top 20 variables with 3 predictors for CI in time 2."
)
